...
from ApartmentManager.serializer import ApartementSerializer


class UserSerializer(serializers.ModelSerializer):
    ''' User serializer'''
    password = serializers.CharField(write_only=True, required=False)
    apartment = ApartementSerializer()

    class Meta:
        model = User
        fields = ['email', 'first_name', 'last_name', 'password', 'provider', 'apartment']
        # required = ['password']
